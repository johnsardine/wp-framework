...
class js_meta_box {
	...
	static function js_css() {
		...
		wp_enqueue_style('js-meta-box', $base_url . '/meta-box.css');
		wp_enqueue_script('js-meta-box', $base_url . '/meta-box.js', array('jquery'), null, true);
	}
	...
	function check_field_upload() {
		...
		add_action('wp_ajax_js_delete_file', array(&$this, 'delete_file'));			// ajax delete files
		add_action('wp_ajax_js_reorder_images', array(&$this, 'reorder_images'));	// ajax reorder images
	}
	...
	function insert_images() {
		if (!isset($_POST['js-insert']) || empty($_POST['attachments'])) return;

		check_admin_referer('media-form');

		$nonce = wp_create_nonce('js_ajax_delete');
		...
		foreach ($_POST['attachments'] as $attachment_id => $attachment) {
			...
			$li .= "<a title='" . __('Delete this image') . "' class='js-delete-file' href='#' rel='$nonce|$post_id|$id|$attachment_id'>" . __('Delete') . "</a>";
	...
	function delete_file() {
		...
		if (!wp_verify_nonce($nonce, 'js_ajax_delete')) die('1');
	...
	function reorder_images() {
		...
		if (!wp_verify_nonce($nonce, 'js_ajax_reorder')) die('1');
	...
	function check_field_date() {
		if ($this->has_field('date') && self::is_edit_page()) {
			...
			wp_enqueue_style('js-jquery-ui-css', 'http://ajax.googleapis.com/ajax/libs/jqueryui/' . self::get_jqueryui_ver() . '/themes/base/jquery-ui.css');
			wp_enqueue_script('js-jquery-ui', 'https://ajax.googleapis.com/ajax/libs/jqueryui/' . self::get_jqueryui_ver() . '/jquery-ui.min.js', array('jquery'));
		}
	}
	...
	function check_field_time() {
		if ($this->has_field('time') && self::is_edit_page()) {
			...
			wp_enqueue_style('js-jquery-ui-css', 'http://ajax.googleapis.com/ajax/libs/jqueryui/' . self::get_jqueryui_ver() . '/themes/base/jquery-ui.css');
			wp_enqueue_script('js-jquery-ui', 'https://ajax.googleapis.com/ajax/libs/jqueryui/' . self::get_jqueryui_ver() . '/jquery-ui.min.js', array('jquery'));
			wp_enqueue_script('js-timepicker', 'https://github.com/trentrichardson/jQuery-Timepicker-Addon/raw/master/jquery-ui-timepicker-addon.js', array('js-jquery-ui'));
		}
	}
	...
	function show() {
		...
		wp_nonce_field(basename(__FILE__), 'js_meta_box_nonce');
	...
	function show_field_begin($field, $meta) {
		echo "<th class='js-label'><label for='{$field['id']}'><strong>{$field['name']}</strong></label></th><td class='js-field'>";
	}
	...
	function show_field_begin_wide($field, $meta) {
		echo "<th class='js-label js-separator' colspan='2'><label for='{$field['id']}'><strong>{$field['name']}</strong></label></th>";
	}
	
	function show_field_begin_special($field, $meta) {
		echo "<th class='js-label js-description' colspan='2'><label for='{$field['id']}'><strong>{$field['name']}</strong></label>";
	}
	
	function show_field_end_special($field, $meta) {
		echo "</th>";
	}

	function show_field_text($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<input type='text' class='js-text' name='{$field['id']}' id='{$field['id']}' value='$meta' size='30' />";
		$this->show_field_end($field, $meta);
	}
	
	function show_field_description($field, $meta) {
		$this->show_field_begin_special($field, $meta);
		echo "<span>'{$field['desc']}'</span>";
		$this->show_field_end_special($field, $meta);
	}
	
	function show_field_separator($field, $meta) {
		$this->show_field_begin_wide($field, $meta);
	}

	function show_field_textarea($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<textarea class='js-textarea large-text' name='{$field['id']}' id='{$field['id']}' cols='60' rows='10'>$meta</textarea>";
		$this->show_field_end($field, $meta);
	}
	...
	function show_field_select($field, $meta) {
		...
		$this->show_field_begin($field, $meta);
		echo "<select class='js-select' name='{$field['id']}" . ($field['multiple'] ? "[]' id='{$field['id']}' multiple='multiple'" : "'") . ">";
	...
	function show_field_radio($field, $meta) {
		$this->show_field_begin($field, $meta);
		foreach ($field['options'] as $key => $value) {
			echo "<input type='radio' class='js-radio' name='{$field['id']}' value='$key'" . checked($meta, $key, false) . " /> $value ";
		}
		$this->show_field_end($field, $meta);
	}
	...
	function show_field_checkbox($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<input type='checkbox' class='js-checkbox' name='{$field['id']}' id='{$field['id']}'" . checked(!empty($meta), true, false) . " /> {$field['desc']}</td>";
	}

	function show_field_wysiwyg($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<textarea class='js-wysiwyg theEditor large-text' name='{$field['id']}' id='{$field['id']}' cols='60' rows='10'>$meta</textarea>";
		$this->show_field_end($field, $meta);
	}
	...
	function show_field_file($field, $meta) {
		...
		if (!empty($meta)) {
			...
			echo '<ol class="js-upload">';
			foreach ($meta as $att) {
				// if (wp_attachment_is_image($att)) continue; // what's image uploader for?
				echo "<li>" . wp_get_attachment_link($att, '' , false, false, ' ') . " (<a class='js-delete-file' href='#' rel='$nonce|{$post->ID}|{$field['id']}|$att'>" . __('Delete') . "</a>)</li>";
			}
			echo '</ol>';
		}
		...
		echo "<div style='clear: both'><strong>" . __('Upload new files') . "</strong></div>
			<div class='new-files'>
				...
				<a class='js-add-file' href='#'>" . __('Add more file') . "</a>
			</div>
		</td>";
	}
	...
	function show_field_image($field, $meta) {
		...
		$nonce_delete = wp_create_nonce('js_ajax_delete');
		$nonce_sort = wp_create_nonce('js_ajax_reorder');

		echo "<input type='hidden' class='js-images-data' value='{$post->ID}|{$field['id']}|$nonce_sort' />
			  <ul class='js-images js-upload' id='js-images-{$field['id']}'>";
		...
		if (!empty($meta)) {
			...
			foreach ($images as $image) {
				...
				echo "<li id='item_$image'>
						<img src='$src' />
						<a title='" . __('Delete this image') . "' class='js-delete-file' href='#' rel='$nonce_delete|{$post->ID}|{$field['id']}|$image'>" . __('Delete') . "</a>
		...
		echo "<a href='#' class='js-upload-button button' rel='{$post->ID}|{$field['id']}'>" . __('Add more images') . "</a>";
	}

	function show_field_color($field, $meta) {
		if (empty($meta)) $meta = '#';
		$this->show_field_begin($field, $meta);
		echo "<input class='js-color' type='text' name='{$field['id']}' id='{$field['id']}' value='$meta' size='8' />
			  <a href='#' class='js-color-select' rel='{$field['id']}'>" . __('Select a color') . "</a>
			  <div style='display:none' class='js-color-picker' rel='{$field['id']}'></div>";
		$this->show_field_end($field, $meta);
	}
	...
	function show_field_checkbox_list($field, $meta) {
		...
		$html = array();
		foreach ($field['options'] as $key => $value) {
			$html[] = "<input type='checkbox' class='js-checkbox_list' name='{$field['id']}[]' value='$key'" . checked(in_array($key, $meta), true, false) . " /> $value";
		}
	...
	function show_field_date($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<input type='text' class='js-date' name='{$field['id']}' id='{$field['id']}' rel='{$field['format']}' value='$meta' size='30' />";
		$this->show_field_end($field, $meta);
	}

	function show_field_time($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<input type='text' class='js-time' name='{$field['id']}' id='{$field['id']}' rel='{$field['format']}' value='$meta' size='30' />";
		$this->show_field_end($field, $meta);
	}
	...
	function save($post_id) {
		...
		|| (!check_admin_referer(basename(__FILE__), 'js_meta_box_nonce'))		// verify nonce
		...
		foreach ($this->_fields as $field) {
			...
			// validate meta value
			if (class_exists('js_meta_box_Validate') && method_exists('js_meta_box_Validate', $field['validate_func'])) {
				$new = call_user_func(array('js_meta_box_Validate', $field['validate_func']), $new);
			}

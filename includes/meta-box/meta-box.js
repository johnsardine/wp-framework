jQuery(document).ready(function($) {
	...
	$('.js-color-select').click(function(){
		$(this).siblings('.js-color-picker').toggle();
		return false;
	});
	...
	// delete file
	$('.js-upload').delegate('.js-delete-file', 'click' , function(){
		var $this = $(this),
			$parent = $this.parent(),
			data = $this.attr('rel');
		$.post(ajaxurl, {action: 'js_delete_file', data: data}, function(response){
	...
	$('.js-images').each(function(){
		...
		$this.sortable({
			...
			update: function (){
				...
				data = order + '|' + $this.siblings('.js-images-data').val();

				$.post(ajaxurl, {action: 'js_reorder_images', data: data}, function(response){
	...
	$('.js-upload-button').click(function(){
		...
		window.send_to_editor = function(html) {
			$('#js-images-' + field_id).append($(html));
	...
	$('.ml-submit:first').append('<input type="hidden" name="field_id" value="' + field_id + '" /> <input type="submit" class="button" name="js-insert" value="Insert selected images" />');
